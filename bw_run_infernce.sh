...
export PREPROCESS_FOLDERS="bridge_01_Day"
...
export INFERENCE_OUTPUT_DIR=$TMPDIR/segcamel/$(date +"%d%m%Y_%H%M")_inference_output_bridge_01_Day_$FEAT_MODE
export SEGMENTATION_OUT_DIR=$TMPDIR/segcamel/$(date +"%d%m%Y_%H%M")_unsup_outputs_bridge_01_Day_$FEAT_MODE
export PIPELINE_MODE="inference"

mkdir -p $TRAIN_CHECKPOINTS
mkdir -p $INFERENCE_OUTPUT_DIR
cp --verbose $HERCULES_PROCESSED/22092025_0406_segcamel_train_output_epoch_50_$FEAT_MODE/checkpoints/best_model.pth $TRAIN_CHECKPOINTS/
export TRAIN_CHECKPOINT_PTH=$TRAIN_CHECKPOINTS/best_model.pth

python -m scripts.preprocess
python -m scripts.segment_once

cp -r $INFERENCE_OUTPUT_DIR/ $HERCULES_PROCESSED/22092025_0406_segcamel_train_output_epoch_50_$FEAT_MODE/
cp -r $SEGMENTATION_OUT_DIR/ $HERCULES_PROCESSED/22092025_0406_segcamel_train_output_epoch_50_$FEAT_MODE/
