...
CLIENT_SECRETS_FILE = "hercules_dataset_download/hercules.json"
...
def get_google_auth_user_info():
    creds = None
    if os.path.exists('hercules_dataset_download/token.pickle'):
        with open('hercules_dataset_download/token.pickle', 'rb') as token:
...
def main():
    to_download = toml.load("hercules_dataset_download/to_download.toml")
