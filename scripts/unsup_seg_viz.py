...
PREFETCH_BUFFER  = 2          # prefetch next N frames from disk
USE_FP16_MATMUL  = True       # FP16 on CUDA for @; softmax still in FP32
...
def _o3d_show_and_snapshot(xyz: np.ndarray, rgb: np.ndarray, title: str, png_path: Optional[Path]):
    if not _HAS_O3D or not SAVE_PNG:
        return
...
def main():
    ...
    if MODE in ("fit_only", "fit_and_apply"):
        ...
        save_run_config(RUN_CFG_JSON, {
            "K": K, "feature_cfg": FEATURE_CFG,
            "dist_edges": DIST_EDGES, "dist_ratios": DIST_RATIOS,
            "soft_assign": SOFT_ASSIGN, "tau": TAU, "conf_th": CONF_TH,
            "vis_aware_smooth": VIS_AWARE_SMOOTH, "vis_vote_mode": VIS_VOTE_MODE,
            "smooth_iters": SMOOTH_ITERS, "neighbor_range": NEIGHBOR_RANGE, "min_component": MIN_COMPONENT,
            "bin_neighbor_ranges": BIN_NEIGHBOR_RANGES, "bin_min_components": BIN_MIN_COMPONENTS,
            "outlier_prune": OUTLIER_PRUNE, "outlier_frac": OUTLIER_FRAC,
            "prefetch_buffer": PREFETCH_BUFFER, "use_fp16_matmul": USE_FP16_MATMUL,
        })
        if MODE == "fit_only":
            print("[fit_only] Done.")
            return
        ...
        if MODE == "apply_only":
            ...
            if RUN_CFG_JSON.exists():
                ...
                globals().update({
                    ...
                    "PREFETCH_BUFFER": cfg.get("prefetch_buffer", PREFETCH_BUFFER),
                    "USE_FP16_MATMUL": cfg.get("use_fp16_matmul", USE_FP16_MATMUL),
                })
