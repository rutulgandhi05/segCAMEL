...
def load_hercules_dataset_folder(dataset_folder: Path, return_all_fields=False, max_workers: int = None):
    ...
    print(f"[DEBUG] Loaded stereo left intrinsics from {stereo_left_yaml}")
    stereo_left_intr, _ = _load_intrinsics_yaml(stereo_left_yaml)
    print(f"[DEBUG] stereo_left_intr:\n{stereo_left_intr}")
    stereo_right_yaml = _first_existing_path(
        calib_folder / "stereo_right.yaml",
        calib_folder / "Stereo_right.yaml",
        calib_folder / "Stereo_Right.yaml",
    )
    print(f"[DEBUG] Loaded stereo right intrinsics from {stereo_right_yaml}")
    stereo_right_intr, _ = _load_intrinsics_yaml(stereo_right_yaml)
    ...
    print(f"[DEBUG] lidar_to_left_ext:\n{lidar_to_left_ext}")
    print(f"[DEBUG] lidar_to_right_ext:\n{lidar_to_right_ext}")
